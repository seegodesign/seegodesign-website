...
export function generatePDFSummary(priorities: Priority[]): void {
  ...
  doc.text('Website Fix Priority Summary', PAGE_MARGIN, cursorY);
  cursorY += 8;

  doc.setFont('helvetica', 'normal');
  doc.setFontSize(10);
  doc.setTextColor(brand.muted);
  doc.text('Generated by Seego Design • Website Fix Priority Engine', PAGE_MARGIN, cursorY);
  cursorY += SECTION_GAP;
  ...
  priorities.forEach((priority, index) => {
    ...
    doc.setFontSize(10);
    doc.setTextColor(brand.ink);
    doc.text('Avoid this:', PAGE_MARGIN, cursorY);
    cursorY += 4.5;

    doc.setFont('helvetica', 'normal');
    doc.setTextColor(brand.muted);
    priority.avoidThis.forEach((item) => {
  ...
  doc.setFontSize(12);
  doc.setTextColor(brand.ink);
  doc.text('What to Ignore for Now', PAGE_MARGIN, cursorY);
  cursorY += 6;
  ...
  const ignoreItems = [
    ...
    "Don't chase every optimization at once",
    "Don't add new features before fixing core issues",
    "Don't delay action waiting for perfect information",
  ];
  ignoreItems.forEach((item) => {
    doc.text(`• ${item}`, PAGE_MARGIN, cursorY);
    cursorY += 5;
  });
  ...
  doc.setTextColor(brand.ink);
  const steps = [
    'Review your #1 priority above',
    'Allocate 2-4 hours to implement the core fix',
    'Test the change on both desktop and mobile',
    'Measure impact before moving to priority #2',
  ];
  ...
  const ctaLines = wrapText(
    doc,
    'Want expert help? Consider booking a One-Day Website Fix to tackle all three priorities in a single focused session.',
    contentWidth,
  );
  doc.text(ctaLines, PAGE_MARGIN, cursorY);

  doc.save('website-fix-priorities.pdf');
}
